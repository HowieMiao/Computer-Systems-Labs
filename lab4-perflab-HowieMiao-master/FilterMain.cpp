...
applyFilter(class Filter *filter, cs1300bmp *input, cs1300bmp *output)
{
  ...
  //create individual variables to reduce calculation
  int iwidth = (input -> width) - 1;
  int iheight = (input -> height) - 1;
  int div = filter -> getDivisor();
    
  //make constants for the three planes
  int outColor1 = 0;
  int outColor2 = 0;
  int outColor3 = 0;
  ...
  int filter22 = filter -> get(2, 2);
    ...
    #pragma omp parallel for //Added openMP to run outer loop on multiple threads
    for(int row = 1; row < iheight; row = row + 1) {  
      ...
      for(int col = 1; col < iwidth; col = col + 1) {
        ...
        if (div != 1) { //Added if because only 2 out of the 4 filters have numbers other than 1
          outColor1 /= div;
          outColor2 /= div;
          outColor3 /= div;
        }
        ...
        //assign values back into output
